				...
				<div class="col-lg-8">
					<div class="breadcrumb_iner">
						<div class="breadcrumb_iner_item">
							<h2>Shop Category</h2>
							<p>${param.proCate}<span>-</span>${param.proSubCate}</p>
						</div>
					</div>
				</div>
			</div>
							...
							<div class="col-lg-4 col-sm-6">
								<form action="addCart.do?status=${status.index}" method="post"
									...
									<input type="hidden" name="id" value="${user.id}"> <input
										...
										type="hidden" name="proFee" value="${v.proFee}"> <input
										...
										<div class="single_product_text">
											...
											<c:if test="${empty user || !empty manager}">
												<input type="button" class="add_cart" value="+add to cart"
													onclick="alert('회원로그인을 먼저 해주세요.')" style="color: #ff3368; text-transform: uppercase; border: none; background-color: white;">
												<!-- <button type="disable"  class="btn_3">add to cart</button> -->
											</c:if>
											<c:if test="${!empty user}">
												<button type="submit" class="add_cart"
													style="color: #ff3368; text-transform: uppercase; border: none; background-color: white;">+
													add to cart</button>
											</c:if>
										</div>
									</div>
